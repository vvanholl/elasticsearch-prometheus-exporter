...
public class PrometheusMetricsCollector {
    ...
    private static final String PROMETHEUS_INDICES_KEY = "prometheus.indices";
    private static final Boolean PROMETHEUS_INDICES_DEFAULT = true;
    ...
    private void registerMetrics() {
        ...
        registerFsMetrics();
        if (settings.getAsBoolean(PROMETHEUS_INDICES_KEY, PROMETHEUS_INDICES_DEFAULT)) registerPerIndexMetrics();
    }
    ...
    private void registerPerIndexMetrics() {
        ...
        catalog.registerGauge("index_store_throttle_time_seconds", "Time spent while storing into indices when throttling", "node", "index", "context");
    ...
    public void updatePerIndexContextMetrics(String index_name, String context, CommonStats idx) {
        ...
        catalog.setGauge("index_store_throttle_time_seconds", idx.getStore().getThrottleTime().millis() / 1000.0, node, index_name, context);
        ...
        catalog.setGauge("index_suggest_current_number", idx.getSuggest().getCurrent(), node, index_name, context);
        catalog.setGauge("index_suggest_count", idx.getSuggest().getCount(), node, index_name, context);
        catalog.setGauge("index_suggest_time_seconds", idx.getSuggest().getTimeInMillis() / 1000.0, node, index_name, context);
    ...
    public void updateMetrics() {
        ...
        if (settings.getAsBoolean(PROMETHEUS_INDICES_KEY, PROMETHEUS_INDICES_DEFAULT)) {
            ...
            try {
                ...
                logger.warn("Could not get indices statistics", e);
            }
        }
